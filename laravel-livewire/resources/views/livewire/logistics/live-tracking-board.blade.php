<div class="space-y-4">
    <div wire:ignore id="live-tracking-map" class="h-80 w-full rounded-2xl border border-slate-200/70 bg-slate-100 dark:border-slate-800/70 dark:bg-slate-900/40"></div>

    @if(empty($markers))
        <p class="text-sm text-slate-600 dark:text-slate-300">{{ __('Sin posiciones recientes para visualizar en el mapa.') }}</p>
    @else
        <ul class="grid gap-3 text-sm text-slate-600 dark:text-slate-300 sm:grid-cols-2">
            @foreach ($markers as $marker)
                <li class="rounded-xl border border-slate-200/60 bg-white/80 p-3 shadow-sm dark:border-slate-800/60 dark:bg-slate-950/60">
                    <p class="font-semibold text-slate-900 dark:text-slate-100">{{ $marker['truck'] ?? __('Vehículo sin placa') }}</p>
