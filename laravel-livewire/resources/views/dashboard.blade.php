<x-layouts.app :title="__('Dashboard')">
 ...
 <article class="relative overflow-hidden rounded-3xl border border-[color:var(--color-primary-border)] bg-gradient-to-br from-[color:var(--color-elevated)] via-[color:var(--color-primary-muted)] to-[color:var(--color-primary-100)] p-8 text-token shadow-lg transition-colors duration-300 ">
 ...
 <a href="{{ route('billing.invoices.index') }}" class="group surface-muted flex transform items-center justify-between px-4 py-3 text-sm font-semibold text-token transition hover:-translate-y-0.5 hover:border-[color:var(--color-primary-border)] hover:[background-color:var(--color-elevated)] ">
 <span>Facturas</span>
 <span class="text-accent-soft transition group-hover:translate-x-0.5 ">&rarr;</span>
 </a>
 <a href="{{ route('billing.payments.index') }}" class="group surface-muted flex transform items-center justify-between px-4 py-3 text-sm font-semibold text-token transition hover:-translate-y-0.5 hover:border-[color:var(--color-primary-border)] hover:[background-color:var(--color-elevated)] ">
 <span>Pagos</span>
 <span class="text-accent-soft transition group-hover:translate-x-0.5 ">&rarr;</span>
 </a>
 </div>
 <div class="mt-6 flex items-center gap-3 text-xs font-medium uppercase tracking-wide text-token-muted ">
                <span class="inline-flex h-2 w-2 rounded-full [background-color:var(--color-success-400)]"></span>
 ...
 <h3 class="text-xl font-semibold text-token ">Disponibilidad y mantenimiento asegurados</h3>
 <p class="text-sm text-token ">Visualiza la salud de cada vehículo y el desempeño de tus conductores sin perder detalle.</p>
 </header>
 <ul class="mt-4 space-y-3 text-sm text-token ">
 <li class="flex items-start gap-2">
 ...
 <h3 class="text-xl font-semibold text-token ">Ingresos bajo control</h3>
 <p class="text-sm text-token ">Genera facturas, registra pagos y analiza resultados financieros con claridad.</p>
 </header>
 <ul class="mt-4 space-y-3 text-sm text-token ">
 <li class="flex items-start gap-2">
 ...
 <div>
 <h3 class="text-lg font-semibold text-token ">Camiones disponibles</h3>
 <p class="mt-1 text-sm text-token ">Últimas unidades listas para despacho.</p>
 </div>
 ...
 <div class="flex flex-wrap items-start justify-between gap-4">
 <div>
 <h3 class="text-lg font-semibold text-token ">Mantenimientos programados</h3>
 <p class="mt-1 text-sm text-token ">Próximas intervenciones agendadas.</p>
 </div>
            ...
            <tbody>
              @foreach(\App\Models\Maintenance::where('status', 'scheduled')->with('truck')->orderBy('maintenance_date')->take(5)->get() as $maintenance)
                <tr class="table-row table-row-hover">
                  ...
                  <td class="table-cell text-sm text-token ">{{ $maintenance->maintenance_type }}</td>
                </tr>
              @endforeach
            </tbody>
          </table>
