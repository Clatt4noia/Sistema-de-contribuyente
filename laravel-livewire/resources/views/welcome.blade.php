    ...
    <body class="min-h-screen bg-slate-50 text-slate-700 antialiased">
        <div class="relative overflow-hidden">
            <div class="absolute inset-x-0 top-0 -z-10 flex justify-center">
                <div class="h-64 w-full max-w-4xl bg-gradient-to-r from-sky-200 via-sky-100 to-indigo-100 blur-3xl"></div>
            </div>

            <header class="mx-auto flex max-w-6xl items-center justify-between px-6 py-10">
                <div class="flex items-center gap-3">
                    <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-500 text-white shadow-lg shadow-sky-200">
                    ...
                    <nav class="flex items-center gap-3 text-sm font-semibold">
                        ...
                        @else
                            ...
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="rounded-xl bg-sky-500 px-4 py-2 text-white shadow-sm transition hover:bg-sky-600">{{ __('Register') }}</a>
                            @endif
                        @endauth
                    </nav>
                @endif
            ...
            <main class="mx-auto max-w-5xl px-6 pb-24">
                <section class="rounded-3xl border border-slate-200 bg-white p-12 text-center shadow-xl shadow-slate-200">
                    ...
                    <div class="mt-8 flex flex-wrap justify-center gap-3">
                        <a href="{{ route('register') }}" class="inline-flex items-center gap-2 rounded-xl bg-sky-500 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-600">
                            {{ __('Comenzar ahora') }}
                        </a>
                        <a href="{{ route('login') }}" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-6 py-3 text-sm font-semibold text-slate-600 transition hover:border-sky-300 hover:text-slate-900">
                ...
                <section class="mt-16 grid gap-6 md:grid-cols-3">
                    <article class="surface-card p-6 text-left">
                        <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-sky-100 text-sky-600">📦</span>
                    ...
                    <article class="surface-card p-6 text-left">
                        <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-100 text-emerald-600">🚚</span>
                    ...
                    <article class="surface-card p-6 text-left">
                        <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-100 text-indigo-600">📈</span>
