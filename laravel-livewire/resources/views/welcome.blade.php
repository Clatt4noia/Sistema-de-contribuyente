    ...
    <body class="min-h-screen bg-surface text-token antialiased">
        <div class="relative overflow-hidden">
            ...
            <header class="mx-auto flex max-w-6xl items-center justify-between px-6 py-10">
                <div class="flex items-center gap-3 text-token">
                    ...
                    <span class="text-lg font-semibold text-token">{{ config('app.name', 'Laravel') }}</span>
                </div>
                @if (Route::has('login'))
                    <nav class="flex items-center gap-3 text-sm font-semibold">
                        @auth
                            <a href="{{ url('/dashboard') }}" class="rounded-xl bg-elevated px-4 py-2 text-token shadow-sm transition hover:text-token hover:shadow">{{ __('Dashboard') }}</a>
                        @else
                            <a href="{{ route('login') }}" class="rounded-xl bg-elevated px-4 py-2 text-token shadow-sm transition hover:text-token hover:shadow">{{ __('Log in') }}</a>
            ...
            <main class="mx-auto max-w-5xl px-6 pb-24">
                ...
                <section class="mt-16 grid gap-6 md:grid-cols-3">
                    ...
                    <article class="surface-card p-6 text-left">
                        ...
                        <h2 class="mt-4 text-lg font-semibold text-token">{{ __('Gestión de flota') }}</h2>
                        <p class="mt-2 text-sm text-token">{{ __('Controla disponibilidad, mantenimientos y documentos clave de cada vehículo.') }}</p>
                    </article>
                    <article class="surface-card p-6 text-left">
                        <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-accent-soft text-accent">📈</span>
                        <h2 class="mt-4 text-lg font-semibold text-token">{{ __('Finanzas en orden') }}</h2>
                        <p class="mt-2 text-sm text-token">{{ __('Centraliza facturas, pagos y métricas con reportes claros y accionables.') }}</p>
                    </article>
                </section>
            </main>
        </div>
